...
def vgg(data_shapes, model_configs):
    ...
    for dim in vgg:
        if dim == 'M':
            ...
            x = layers.Conv1D(int(dim/div), kernel_size=3, strides=1, activation='elu', padding='same', kernel_initializer=init, kernel_regularizer=reg)(x)
            #x = layers.BatchNormalization()(x)
    ...
    for dim in dense:
        x = layers.Dense(int(dim/div), activation='elu', kernel_initializer=init, kernel_regularizer=reg)(x)
        #x = layers.BatchNormalization()(x)
        #x = layers.Activation('elu')(x)
        #x = layers.Dropout(0.3)(x)
